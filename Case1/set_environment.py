...
class Environment(gym.Env):
    ...
    def __init__(
        ...
        dim=3.5,
        # ===== 新增磁控相关参数 =====
        use_magnetic_actuation=False,  # 是否启用磁力矩
        magnetic_field_strength=0.0,   # 磁场强度（80 mT，默认关闭）
        magnetic_angular_frequency=0.0,  # 旋转磁场频率
        magnetization_density=0.0,     # 磁化强度
        magnetization_direction=np.array([0.0, 1.0, 0.0]),  # 默认沿 y 轴磁化
        magnetic_ramp_interval=0.0,    # ramp 时间，0 为常值磁场
        magnetic_start_time=0.0,      # 磁场启动时间
        magnetic_end_time=5e3,        # 磁场结束时间
        use_rotating_field=False,      # 是否使用旋转磁场
        *args,
        **kwargs,
    ):
        """
        ...
        **kwargs
        ...
        self.n_elem = n_elem
                ...
                # ======= 可选：磁控相关参数（默认不开启磁控，不影响原来 Case1） =======
        # 是否启用磁力矩
        self.use_magnetic_actuation = kwargs.get("use_magnetic_actuation", False)

        # 外磁场强度（比如 80e-3 -> 80 mT）
        self.magnetic_field_strength = kwargs.get("magnetic_field_strength", 0.0)

        # 如果想用旋转磁场，这里是角频率（rad/s），类似前面磁毯例子里的 angular_frequency
        self.magnetic_angular_frequency = kwargs.get("magnetic_angular_frequency", 0.0)

        # 磁化强度（磁矩 = density * volume），你可以根据磁毯例子里的公式自己算，也可以直接给个数
        self.magnetization_density = kwargs.get("magnetization_density", 0.0)

        # 磁化方向（世界坐标系），默认沿 y 轴
        self.magnetization_direction = np.asarray(
            kwargs.get("magnetization_direction", np.array([0.0, 1.0, 0.0])),
            dtype=float,
        )

        # 磁场 ramp 的时间参数（可选）
        self.magnetic_ramp_interval = float(kwargs.get("magnetic_ramp_interval", 0.0))
        self.magnetic_start_time = float(kwargs.get("magnetic_start_time", 0.0))
        self.magnetic_end_time = float(
            kwargs.get("magnetic_end_time", self.final_time)
        )

        # 是否使用旋转磁场（True -> SingleModeOscillatingMagneticField，False -> ConstantMagneticField）
        self.use_rotating_field = bool(kwargs.get("use_rotating_field", False))
    ...
    def reset(self):
        ...
        self.simulator.constrain(self.shearable_rod).using(
            OneEndFixedRod, constrained_position_idx=(0,), constrained_director_idx=(0,)
                ...
                # ======= 可选：给 arm 施加磁力矩（如果在 __init__ 中开启了 use_magnetic_actuation） =======
        if getattr(self, "use_magnetic_actuation", False) and self.magnetic_field_strength > 0.0:

            # 1) 先构造一个外磁场对象（常值 or 旋转）
            if self.use_rotating_field and self.magnetic_angular_frequency != 0.0:
                # 例子：类似磁毯用的 3D 旋转场，稍微 tilt 一点 y 分量
                magnetic_field = SingleModeOscillatingMagneticField(
                    magnetic_field_amplitude=self.magnetic_field_strength
                    * np.array([1.0, 1e-2, 1.0]),
                    magnetic_field_angular_frequency=np.array(
                        [self.magnetic_angular_frequency, 0.0, self.magnetic_angular_frequency]
                    ),
                    magnetic_field_phase_difference=np.array([0.0, np.pi / 2.0, np.pi / 2.0]),
                    ramp_interval=self.magnetic_ramp_interval,
                    start_time=self.magnetic_start_time,
                    end_time=self.magnetic_end_time,
                )
            else:
                # 简单常值磁场：方向取你给的 magnetization_direction 方向
                B_dir = self.magnetization_direction
                ...
                magnetic_field = ConstantMagneticField(
                    magnetic_field_amplitude=self.magnetic_field_strength * B_dir,
                    ramp_interval=self.magnetic_ramp_interval,
                    start_time=self.magnetic_start_time,
                    end_time=self.magnetic_end_time,
                )

            # 2) 把 MagneticForces 挂到 rod 上
            self.simulator.add_forcing_to(self.shearable_rod).using(
                MagneticForces,
                external_magnetic_field=magnetic_field,
                magnetization_density=self.magnetization_density,
                magnetization_direction=self.magnetization_direction,
                rod_volume=self.shearable_rod.volume,
                rod_director_collection=self.shearable_rod.director_collection,
            )
    ...
    def step(self, action):
        ...
        self.previous_action = action
            ...
            # 当前的磁力矩
        if self.use_magnetic_actuation and self.magnetic_field_strength > 0.0:
            print("Magnetic Force Torque: ", self.shearable_rod.external_torques)
