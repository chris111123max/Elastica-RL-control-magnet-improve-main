...
env = Environment(
    ...
    max_rate_of_change_of_activation=max_rate_of_change_of_activation,
        # ===== 新增：磁控参数 =====
    use_magnetic_actuation=use_magnetic,
    magnetic_field_strength=80e-3,                    # 80 mT
    magnetic_angular_frequency=np.deg2rad(10.0),      # 如果你想要旋转场
    use_rotating_field=True,                          # True -> 用 SingleModeOscillatingMagneticField
    magnetization_density=1e4,                        # 随便先给个量级，你可以按文献公式调
    magnetization_direction=np.array([0.0, 1.0, 0.0]),
    magnetic_ramp_interval=0.01,                      # 前 0.01s 线性 ramp
)

name = str(args.algo_name) + "_3d-tracking_id"
mag_suffix = "_mag" if use_magnetic else "_nomag"
identifer = (
    name
    + mag_suffix
    + "-"
